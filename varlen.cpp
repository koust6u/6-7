...
{
	int recaddr = stream.tellp();//return the current location of put pointer of the stream
	unsigned short bufferSize;
	bufferSize = BufferSize;
	...
	return recaddr;//return the address in the stream where the record was written
}
...
{
	//char str[headerSize+1];//visual studio 지원 안함
	char str[9];
	int result;
